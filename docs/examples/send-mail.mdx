...
## Sending mail with runScript

Import `runScript` from `@eney/api` and use it inside your `sendMail` helper to execute the script.


```typescript
import { runScript, useCloseWidget } from "@eney/api";
...
- Import `runScript` from `@eney/api` and call it directly inside your async helpers
