...
1. **Interacting with macOS apps** using the [`runScript`](/docs/utils/use-apple-script) function to create and manage content.
...
## Writing notes with runScript

`runScript` is imported from `@eney/api` and used directly within the async `createNote` helper to execute AppleScript commands:
...
- Use the `useLogger` for debug output. This keeps your logs in the developer console and prevents them from cluttering the extension's UI.
