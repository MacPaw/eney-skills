...
1. **Writing to macOS apps** with [`useAppleScript`](/docs/hooks/use-apple-script) — creating notes through AppleScript
...
The widget lets the user select a folder from a dropdown (populated from the Notes database), enter a note name, and write rich text content using a rich text editor. Content is converted from Markdown to HTML before being written to Notes as a new note in the selected folder.

---

## Writing notes with useAppleScript

`useAppleScript` is called at the top of the component like any other hook, and `runScript` is used inside the async `createNote` helper defined within the component:
...
- Show a loading state while fetching data, and display inline errors instead of closing the widget on failure
